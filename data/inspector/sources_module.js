...
{WebInspector.SidebarPane.call(this,WebInspector.UIString("Breakpoints"));this.registerRequiredCSS("breakpointsList.css");this._breakpointManager=breakpointManager;this._showSourceLineDelegate=showSourceLineDelegate;this.listElement=document.createElementWithClass("ol","breakpoint-list");this.emptyElement=this.bodyElement.createChild("div","info");this.emptyElement.textContent=WebInspector.UIString("No Breakpoints");this._items=new Map();var breakpointLocations=this._breakpointManager.allBreakpointLocations();for(var i=0;i<breakpointLocations.length;++i)
...
{if(event)
event.consume();this.expand();var inputElementContainer=document.createElementWithClass("p","breakpoint-condition");inputElementContainer.textContent=WebInspector.UIString("Break when URL contains:");var inputElement=inputElementContainer.createChild("span","editing");inputElement.id="breakpoint-condition-input";this.addListElement(inputElementContainer,(this.listElement.firstChild));function finishEditing(accept,e,text)
...
return;var element=document.createElement("li");element._url=url;element.addEventListener("contextmenu",this._contextMenu.bind(this,url),true);var checkboxElement=element.createChild("input","checkbox-elem");checkboxElement.type="checkbox";checkboxElement.checked=enabled;checkboxElement.addEventListener("click",this._checkboxClicked.bind(this,url),false);element._checkboxElement=checkboxElement;var labelElement=element.createChild("span","cursor-auto");if(!url)
...
{var element=this._breakpointElements[url];var inputElement=document.createElementWithClass("span","breakpoint-condition editing");inputElement.textContent=url;this.listElement.insertBefore(inputElement,element);element.classList.add("hidden");function finishEditing(accept,e,text)
...
{var checkbox=document.createElementWithClass("input","checkbox-elem");checkbox.type="checkbox";labelNode.insertBefore(checkbox,labelNode.firstChild);return checkbox;},_categoryCheckboxClicked:function(categoryItem)
...
{this.element=document.createElementWithClass("div","source-frame-infobar source-frame-infobar-"+level);this._mainRow=this.element.createChild("div","source-frame-infobar-main-row");this._detailsContainer=this.element.createChild("span","source-frame-infobar-details-container");this._mainRow.createChild("span","source-frame-infobar-icon");this._mainRow.createChild("span","source-frame-infobar-row-message").textContent=message;this._toggleElement=this._mainRow.createChild("div","source-frame-infobar-toggle source-frame-infobar-link");this._toggleElement.addEventListener("click",this._onToggleDetails.bind(this),false);this._closeElement=this._mainRow.createChild("div","close-button");this._closeElement.addEventListener("click",this._onClose.bind(this),false);this._updateToggleElement();}
...
return;var url=this._uiSourceCode.url;var isContentScript=projectType===WebInspector.projectTypes.ContentScripts;if(!WebInspector.BlackboxSupport.isBlackboxed(url,isContentScript)){this._hideBlackboxInfobar();return;}
...
return;projectNode.treeElement().titleText=project.displayName();},_folderNode:function(projectNode,folderPath)
{if(!folderPath)
...
WebInspector.SourcesPanel.minToolbarWidth=215;WebInspector.SourcesPanel._infobarSymbol=Symbol("infobar");WebInspector.SourcesPanel.prototype={_setTarget:function(target)
{if(!target)
...
{var uiSourceCode=(event.data);this._editorChanged(uiSourceCode);if(Runtime.experiments.isEnabled("suggestUsingWorkspace")){if(this._editorSelectedTimer)
...
{if(!this.isShowing())
return;if(uiSourceCode[WebInspector.SourcesPanel._infobarSymbol])
return;var uiSourceCodeFrame=this._sourcesView.viewForFile(uiSourceCode);if(uiSourceCode.project().type()===WebInspector.projectTypes.FileSystem){var hasMappings=!!uiSourceCode.url;if(hasMappings)
return;var targets=WebInspector.targetManager.targets();for(var i=0;i<targets.length;++i)
targets[i].resourceTreeModel.forAllResources(matchResource.bind(this));}
function matchResource(resource)
{if(resource.contentURL().endsWith(uiSourceCode.name())){createMappingInfobar.call(this,false);return true;}
return false;}
if(uiSourceCode.project().type()===WebInspector.projectTypes.Network||uiSourceCode.project().type()===WebInspector.projectTypes.ContentScripts){if(this._workspace.uiSourceCodeForURL(uiSourceCode.url)!==uiSourceCode)
return;var filesystemProjects=this._workspace.projectsForType(WebInspector.projectTypes.FileSystem);for(var i=0;i<filesystemProjects.length;++i){var name=uiSourceCode.name();var fsUiSourceCodes=filesystemProjects[i].uiSourceCodes();for(var j=0;j<fsUiSourceCodes.length;++j){if(fsUiSourceCodes[j].name()===name){createMappingInfobar.call(this,true);return;}}}
var originURL=uiSourceCode.originURL().asParsedURL();if(originURL&&originURL.host==="localhost")
