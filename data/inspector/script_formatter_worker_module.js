...
{return callback.apply(null,arg);}};(function(window){window.CodeMirror={};(function(){"use strict";function splitLines(string){return string.split(/\r?\n|\r/);};function StringStream(string){this.pos=this.start=0;this.string=string;this.lineStart=0;}
...
return CodeMirror.Pass;},innerMode:function(state){return{state:state.localState||state.htmlState,mode:state.localMode||htmlMode};}};},"xml","javascript","css");CodeMirror.defineMIME("text/html","htmlmixed");});;FormatterWorker={createTokenizer:function(mimeType)
...
processedChunkCharacters+=newColumn-column;if(processedChunkCharacters>=chunkSize){postMessage({chunk:outlineChunk,isLastChunk:false});outlineChunk=[];processedChunkCharacters=0;}}
...
break;case FormatterWorker.CSSParserStates.PropertyName:if(tokenValue===":"&&tokenType===UndefTokenType){property.name=property.name.trim();state=FormatterWorker.CSSParserStates.PropertyValue;}else if(tokenType["property"]){property.name+=tokenValue;}
...
{this.line=content;this._content=content;this._formattedContent="";this._mapping={original:[0],formatted:[0]};this._position=0;var scriptOpened=false;var styleOpened=false;var tokenizer=FormatterWorker.createTokenizer("text/html");var accumulatedTokenValue="";var accumulatedTokenStart=0;function processToken(tokenValue,tokenType,tokenStart,tokenEnd){if(!tokenType)
return;tokenType=tokenType.split(" ").keySet();if(!tokenType["tag"])
return;if(tokenType["bracket"]&&(tokenValue==="<"||tokenValue==="</")){accumulatedTokenValue=tokenValue;accumulatedTokenStart=tokenStart;return;}
accumulatedTokenValue=accumulatedTokenValue+tokenValue.toLowerCase();if(accumulatedTokenValue==="<script"){scriptOpened=true;}else if(scriptOpened&&tokenValue===">"){scriptOpened=false;this._scriptStarted(tokenEnd);}else if(accumulatedTokenValue==="</script"){this._scriptEnded(accumulatedTokenStart);}else if(accumulatedTokenValue==="<style"){styleOpened=true;}else if(styleOpened&&tokenValue===">"){styleOpened=false;this._styleStarted(tokenEnd);}else if(accumulatedTokenValue==="</style"){this._styleEnded(accumulatedTokenStart);}
...
FormatterWorker.JavaScriptFormatter._identifierRegex=/^[$A-Z_][0-9A-Z_$]*$/i;FormatterWorker.JavaScriptFormatter.prototype={format:function()
...
{var next=this._next();if(next!==token)
throw"Unexpected token: expected "+token+", actual "+next;},_expectGeneralIdentifier:function()
{var next=this._next();if(next!==FormatterWorker.JavaScriptTokens.IDENTIFIER&&!FormatterWorker.JavaScriptFormatter._identifierRegex.test(this._token.value))
throw"Unexpected token: expected javascript identifier, actual "+this._token.value;},_expectSemicolon:function()
...
this._consume(FormatterWorker.JavaScriptTokens.CONST);this._builder.addSpace();var isFirstVariable=true;do{if(!isFirstVariable){this._consume(FormatterWorker.JavaScriptTokens.COMMA);this._builder.addSpace();}
...
{this._expect(FormatterWorker.JavaScriptTokens.FOR);this._builder.addSpace();this._expect(FormatterWorker.JavaScriptTokens.LPAREN);if(this._peek()!==FormatterWorker.JavaScriptTokens.SEMICOLON){if(this._peek()===FormatterWorker.JavaScriptTokens.VAR||this._peek()===FormatterWorker.JavaScriptTokens.CONST){this._parseVariableDeclarations();if(this._peek()===FormatterWorker.JavaScriptTokens.IN||(this._peek()===FormatterWorker.JavaScriptTokens.IDENTIFIER&&this._nextToken.value==="of")){this._builder.addSpace();this._next();this._builder.addSpace();this._parseExpression();}}else
...
this._addText("//");else
this._addText("/*");this._addText(comment.value);if(comment.type!=="comment1"){this._addText("*/");var position=-1;while((position=comment.value.indexOf("\n",position+1))!==-1)
...
{this._readNextToken=tokenizerHolder.tokenizer(content);this._state=this._readNextToken.context();}
