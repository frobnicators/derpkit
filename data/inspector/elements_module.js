...
{this._selectedNode=node;this._refreshThrottler.schedule(this._refreshView.bind(this));},_refreshView:function(finishCallback)
...
this._linkifier.reset();var body=this.bodyElement;body.removeChildren();this.sections=[];var node=this._selectedNode;if(!node){finishCallback();return;}
this._lastRequestedNode=node;node.eventListeners(WebInspector.EventListenersSidebarPane._objectGroupName,callback.bind(this));function callback(eventListeners)
{if(!eventListeners){finishCallback();return;}
var selectedNodeOnly="selected"===WebInspector.settings.eventListenersFilter.get();var sectionNames=[];var sectionMap={};for(var i=0;i<eventListeners.length;++i){var eventListener=eventListeners[i];if(selectedNodeOnly&&(node.id!==eventListener.payload().nodeId))
...
finishCallback();}},_refreshButtonClicked:function()
...
{this._node=node;this._refreshThrottler.schedule(this._refreshView.bind(this));},_refreshView:function(finishCallback)
...
var node=this._node;if(!node){this.bodyElement.removeChildren();this.sections=[];finishCallback();return;}
...
return;for(var i=0;i<this.sections[0].length;++i){var section=this.sections[0][i];if(section instanceof WebInspector.ComputedStylePropertiesSection)
section.onShowInheritedChanged();}},_onAddButtonLongClick:function(event)
...
this._parentPane._userOperation=true;var selectedNode=this._parentPane._node;this._parentPane._target.cssModel.setRuleSelector(this.rule,selectedNode?selectedNode.id:0,newContent,successCallback.bind(this),finishOperationAndMoveEditor.bind(this,moveDirection));},_updateRuleOrigin:function()
...
{WebInspector.PropertiesSection.call(this,"");this._hasFreshContent=false;this.element.className="styles-section monospace read-only computed-style";this.headerElement.appendChild(WebInspector.ComputedStylePropertiesSection._showInheritedCheckbox());this.onShowInheritedChanged();this._stylesPane=stylesPane;this.styleRule=styleRule;this._usedProperties=usedProperties;this._alwaysShowComputedProperties={"display":true,"height":true,"width":true};this.computedStyle=true;this._propertyTreeElements={};this._expandedPropertyNames={};}
...
WebInspector.ComputedStylePropertiesSection.prototype={onShowInheritedChanged:function()
{this.element.classList.toggle("styles-show-inherited",WebInspector.settings.showInheritedComputedStyleProperties.get());},collapse:function(dontRememberState)
